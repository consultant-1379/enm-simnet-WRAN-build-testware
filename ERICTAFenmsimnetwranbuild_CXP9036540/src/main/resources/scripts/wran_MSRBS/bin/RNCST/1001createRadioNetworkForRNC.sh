...
IUBEND=$((NUMOFDG2+NUMOFPRBS))
echo "num g dg2 and prbs $IUBEND"
IUBCOUNT=1
...
(
    parent "ManagedElement=1,SystemFunctions=1,Licensing=1"
    identity "AnonymizedEventData"
    moType RncFeature
    exception none
    nrOfAttributes 2
    "RncFeatureId" String "AnonymizedEventData"
    "licenseState" Integer 1
)

CREATE
(
    parent "ManagedElement=1,SystemFunctions=1,Licensing=1,RncFeature=AnonymizedEventData"
    identity "1"
...
(
    ...
    exception none
    nrOfAttributes 1
    "lastUpPiChange" String "161227-115537"
)
...
then
    echo "############################"
    echo "Running script ro set FlexPmStats MO ... "
    echo "############################"

    ./createRncFeature_FlexPmStats.sh $SIMNAME $RNCIDCOUNT
fi
...
###creation of RDT and UeRealTimeTrace MOs 
