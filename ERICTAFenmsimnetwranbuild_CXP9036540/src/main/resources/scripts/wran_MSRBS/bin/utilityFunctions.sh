...
setup_netconf_port() # portname
{
   ...
   # echo ".config deleteport $1"
    echo ".config save"
    echo ".config add port $1 netconf_port `hostname`"
...
do 
  ...
  TOTALNUMOFNODES=`expr $TOTALNUMOFNODES  + $i + $NUMOFDG2 + $NUMOFPICO + 5`
done
...
RNCCOUNT=$3
RNCVERSION=$4
OFFSET=$(cat ./availableip.txt | grep -v ":" | sort | uniq | head -n 1)
...
NUMOFNODE=$4
NODEVERSION=$5
OFFSET=$(cat ./availableip.txt | grep -v ":" | sort | uniq | head -n 2 | tail -1)
...
NUMOFNODE=$4
NODEVERSION=$5
NUMOFNODE=`expr $NUMOFNODE - 1`
OFFSET=$(cat ./availableip.txt | grep -v ":" | sort | uniq | head -n 2 | tail -1)


    echo '.open '$SIM
    echo '.createne checkport '$PORT
    echo '.selectnocallback NE01'
    echo '.clone '$NUMOFNODE' NE 2'
    echo '.selectnetype MSRBS-V2'
    echo '.set port '$PORT
    echo '.createne subaddr '$OFFSET' subaddr no_value'
    echo '.set external '$PORT
    echo '.set save'
    echo '.rename -auto MSRBS-V2 01'
    echo '.set save'
}
startDG2Nodes()
{
...
numofnodes=`echo -e ".open $SIM \n .show simnes" | /netsim/inst/netsim_shell | grep "LTE MSRBS-V2" | cut -d" " -f1 | wc -l`
################## Fetching free ips #########################
