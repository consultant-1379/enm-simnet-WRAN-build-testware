...
JAVAMOFILECOUNT=`expr $COUNT + $NUMOFPICO`
	if [ "$JAVAMOFILECOUNT" -le 9 ]
   then
    RBSNAME1=RBS0
   else
    RBSNAME1=RBS
...
JAVANODENAME=$RNCNAME$RBSNAME1$JAVAMOFILECOUNT.mo
echo "javanodename:: $JAVANODENAME"
#echo $RBSNAME1"  "$JAVAMOFILECOUNT.mo"  "$JAVANODENAME
 NODENAME=$RNCNAME$RBSNAME$COUNT.mo 

 if [ -f  $JARPATH/$RNCNAME/$JAVANODENAME ];
	then echo "File exist"
	cat $JARPATH/$RNCNAME/$JAVANODENAME > $MOSCRIPT$MOFILEEXTENSION
 else
	echo "File not exist"
