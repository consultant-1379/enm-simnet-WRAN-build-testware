#!/bin/sh

if [ "$#" -ne 3  ]
then
cat<<HELP

Usage: $0 <sim name> <env file> <rnc num>

Example: $0 WegaC5LargeRNC14 SIM1.env 9 (to create RNC09)

CREATE: (Managed Element, Equipment)Subrack, 
	(Managed Element, Equipment, Subrack)Slot,
	(Managed Element, Equipment, Subrack, Slot)PluIgnUnit
	(ManagedElement=1,Equipment=1,Subrack=2,Slot=1,PlugInUnit=1)SwitchCoreUnit	

HELP
 exit 1
fi


ENV=$2
SIMNAME=$1


if [ "$3" -le 9 ]
then
RNCNAME="RNC0"$3
RNCCOUNT="0"$3
else
RNCNAME="RNC"$3
RNCCOUNT=$3
fi

if [ "$3" -eq 0 ]
then
RNCNAME=
fi
PWD=`pwd`
source $PWD/../../dat/$ENV


PWD=`pwd`
NOW=`date +"%Y_%m_%d_%T:%N"`

max=1000000
RANDOM=$((`cat /dev/urandom|od -N1 -An -i` % $max))

MOSCRIPT=$SIMDIR/mo/$RBSDIR/subrack2.mo
MMLSCRIPT=$0${NOW}:$$${RANDOM}".mml"


if [ -f $PWD/$MOSCRIPT ]
then
rm -r  $PWD/$MOSCRIPT
echo "old "$PWD/$MOSCRIPT " removed"
fi


if [ -f $PWD/$MMLSCRIPT ]
then
rm -r  $PWD/$MMLSCRIPT
echo "old "$PWD/$MMLSCRIPT " removed"
fi



#########################################
# 
# Make MO Script
#
#########################################

echo ""
echo "MAKING MO SCRIPT"
echo ""




#########################################
#
# Make MML Script
#
#########################################

echo ""
echo "MAKING MML SCRIPT"
echo ""

cat >> $MMLSCRIPT << MMLSCT

.open $SIMNAME
.selectregexp simne $RBSNES
.start
useattributecharacteristics:switch="off";
kertayle:file="$MOSCRIPT";

MMLSCT



$NETSIMDIR/$NETSIMVERSION/netsim_shell < $MMLSCRIPT

rm $PWD/$MMLSCRIPT

