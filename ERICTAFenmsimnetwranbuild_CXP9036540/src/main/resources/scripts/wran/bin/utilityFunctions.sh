...
do 
  ...
  TOTALNUMOFNODES=`expr $TOTALNUMOFNODES + $NUMOFRBS + $i + $NUMOFPICO + 7`
done
echo "*********************************************************"
...
RNCVERSION=$4
#OFFSET=$5
RNCOFFSET=$(cat ./availableip.txt | grep -v ":" | sort | uniq | head -n 1)
...
echo '.set port '$PORT
echo '.createne subaddr '$RNCOFFSET' subaddr no_value'
echo '.set save'
}
...
#    echo '.set netype WCDMA' $NODETYPE $NODEVERSION // commented temporarily for lte prbs support
    echo '.set netype' $NODETYPE $NODEVERSION
